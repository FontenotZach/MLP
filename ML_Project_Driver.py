...
for interval in intervalList:
    ...
    p1MedianHigh = statistics.median_high(p1Float)
    p1MedianLow = statistics.median_low(p1Float)
    p1Var = statistics.pvariance(p1Float)
    p1Max = np.max(p1Float)
    p1Min = np.min(p1Float)
    p1GCD = math.gcd(p1Float)
    p1Trunc = math.trunc(p1Float)
    p1Log = math.log(p1Float)

    p2Float = [x for x in map(float, interval.p2)]
    p2Avg = sum(p2Float) / len(interval.p2)
    p2SD = statistics.pstdev(p2Float)
    p2Mode = statistics.mode(p2Float)
    p2Median = statistics.median(p2Float)
    p2MedianHigh = statistics.median_high(p2Float)
    p2MedianLow = statistics.median_low(p2Float)
    p2Var = statistics.pvariance(p2Float)
    p2Max = np.max(p2Float)
    p2Min = np.min(p2Float)
    p2GCD = math.gcd(p2Float)
    p2Trunc = math.trunc(p2Float)
    p2Log = math.log(p2Float)

    p3Float = [x for x in map(float, interval.p3)]
    p3Avg = sum(p3Float) / len(interval.p3)
    p3SD = statistics.pstdev(p3Float)
    p3Mode = statistics.mode(p3Float)
    p3Median = statistics.median(p3Float)
    p3MedianHigh = statistics.median_high(p3Float)
    p3MedianLow = statistics.median_low(p3Float)
    p3Var = statistics.pvariance(p3Float)
    p3Max = np.max(p3Float)
    p3Min = np.min(p3Float)
    p3GCD = math.gcd(p3Float)
    p3Trunc = math.trunc(p3Float)
    p3Log = math.log(p3Float)

    p4Float = [x for x in map(float, interval.p4)]
    p4Avg = sum(p4Float) / len(interval.p4)
    p4SD = statistics.pstdev(p4Float)
    p4Mode = statistics.mode(p4Float)
    p4Median = statistics.median(p4Float)
    p4MedianHigh = statistics.median_high(p4Float)
    p4MedianLow = statistics.median_low(p4Float)
    p4Var = statistics.pvariance(p4Float)
    p4Max = np.max(p4Float)
    p4Min = np.min(p4Float)
    p4GCD = math.gcd(p4Float)
    p4Trunc = math.trunc(p4Float)
    p4Log = math.log(p4Float)

    p5Float = [x for x in map(float, interval.p5)]
    p5Avg = sum(p5Float) / len(interval.p5)
    p5SD = statistics.pstdev(p5Float)
    p5Mode = statistics.mode(p5Float)
    p5Median = statistics.median(p5Float)
    p5MedianHigh = statistics.median_high(p5Float)
    p5MedianLow = statistics.median_low(p5Float)
    p5Var = statistics.pvariance(p5Float)
    p5Max = np.max(p5Float)
    p5Min = np.min(p5Float)
    p5GCD = math.gcd(p5Float)
    p5Trunc = math.trunc(p5Float)
    p5Log = math.log(p5Float)

    p6Float = [x for x in map(float, interval.p6)]
    p6Avg = sum(p6Float) / len(interval.p6)
    p6SD = statistics.pstdev(p6Float)
    p6Mode = statistics.mode(p6Float)
    p6Median = statistics.median(p6Float)
    p6MedianHigh = statistics.median_high(p6Float)
    p6MedianLow = statistics.median_low(p6Float)
    p6Var = statistics.pvariance(p6Float)
    p6Max = np.max(p6Float)
    p6Min = np.min(p6Float)
    p6GCD = math.gcd(p6Float)
    p6Trunc = math.trunc(p6Float)
    p6Log = math.log(p6Float)

    p7Float = [x for x in map(float, interval.p7)]
    p7Avg = sum(p7Float) / len(interval.p7)
    p7SD = statistics.pstdev(p7Float)
    p7Mode = statistics.mode(p7Float)
    p7Median = statistics.median(p7Float)
    p7MedianHigh = statistics.median_high(p7Float)
    p7MedianLow = statistics.median_low(p7Float)
    p7Var = statistics.pvariance(p7Float)
    p7Max = np.max(p7Float)
    p7Min = np.min(p7Float)
    p7GCD = math.gcd(p7Float)
    p7Trunc = math.trunc(p7Float)
    p7Log = math.log(p7Float)

    p8Float = [x for x in map(float, interval.p8)]
    p8Avg = sum(p8Float) / len(interval.p8)
    p8SD = statistics.pstdev(p8Float)
    p8Mode = statistics.mode(p8Float)
    p8Median = statistics.median(p8Float)
    p8MedianHigh = statistics.median_high(p8Float)
    p8MedianLow = statistics.median_low(p8Float)
    p8Var = statistics.pvariance(p8Float)
    p8Max = np.max(p8Float)
    p8Min = np.min(p8Float)
    p8GCD = math.gcd(p8Float)
    p8Trunc = math.trunc(p8Float)
    p8Log = math.log(p8Float)
